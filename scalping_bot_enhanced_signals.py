...
def signal(df):
    ...
    cond_buy = (
        ...
        last['cci'] < -100 and
        last['close'] > last['sar'] and
        last['close'] > last['vwap'] and
        last['stoch_k'] < 20 and last['stoch_d'] < 20
    )
    cond_sell = (
        last['rsi'] > 70 and
        ...
        last['close'] < last['sar'] and
        last['close'] < last['vwap'] and
        last['stoch_k'] > 80 and last['stoch_d'] > 80
    )
    if cond_buy:
        return 'buy'
    elif cond_sell:
...
while True:
    try:
        ...
        print(f"[{timestamp}] [{symbol}] Price: ${price:.2f} | Signal: {sig} | Qty: {qty}")

        if sig in ['buy', 'sell'] and qty > 0:
            place_order(sig, symbol, qty)
